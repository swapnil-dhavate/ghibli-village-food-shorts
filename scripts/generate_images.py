"""Generate one still image per scene via Pollinations.ai's free, keyless `flux` model.

No character-consistency guarantee here -- same wording every scene plus a shared seed (see
generate_all_images) is the only lever available without a reference image. The kontext
image-to-image reference-conditioning path that used to live here (for a fixed recurring food-
channel cast) is gone: verified directly against a real account that kontext costs ~$0.04/image
with no automatic free grant, and there's no fixed cast to condition on anymore now that every
video covers a different general topic. If a genuinely free reference-conditioned model ever
shows up, this is where it would plug back in.
"""
...
# Deliberately a DIFFERENT model from generate_script.py's GEMINI_MODEL -- each Gemini free-tier
...
# keeps them off the script-generation model's quota entirely.
...
def generate_image(scene, width, height, seed, model, out_path):
    ...
    Gemini's vision QC says the render doesn't match the scene's intended visual. Free-tier
    ...
    for attempt in range(QC_MAX_ATTEMPTS):
        ...
        if check_image_matches(image_bytes, scene["visual_description"]):
    ...
    raise RuntimeError(
        ...
        f"(visual: {scene['visual_description']!r}) -- refusing to publish a confirmed-mismatched image"
    )
